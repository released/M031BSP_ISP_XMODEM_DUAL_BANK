...
uint32_t read_EEP_u32data(uint8_t addr)
{
    uint32_t res = 0;
    uint8_t val = 0;
    
    Read_Data(addr + 0 , &val);
    res |= val;
    Read_Data(addr + 1 , &val);
    res |= val << 8;
    Read_Data(addr + 2 , &val);
    res |= val << 16;
    Read_Data(addr + 3 , &val);
    res |= val << 24;    

    // LDROM_DEBUG("read_EEP_u32data <0x%04X> : 0x%4X\r\n", addr,res); 
    return res;
}

void write_EEP_u32data(uint8_t addr , uint32_t val)
{
    Write_Data( addr + 0 , GET_BYTE0(val) ); 
    Write_Data( addr + 1 , GET_BYTE1(val) );    
    Write_Data( addr + 2 , GET_BYTE2(val) );    
    Write_Data( addr + 3 , GET_BYTE3(val) );     

    // LDROM_DEBUG("write_EEP_u32data <0x%04X> : 0x%4X\r\n", addr,val);    
}
